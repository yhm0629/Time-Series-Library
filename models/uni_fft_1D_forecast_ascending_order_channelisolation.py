...
class Model(nn.Module):
    ...
    def forward(self, x_enc, x_mark_enc=None, x_dec=None, x_mark_dec=None, mask=None):
        ...
        # --- 核心干预 1：提取初始物理空间边界并执行张量折叠 ---
        B, L, C = x_enc.shape
        if self.task_name in ['long_term_forecast', 'short_term_forecast']:
            # (Batch, Length, Channel) -> (Batch * Channel, Length, 1)
            # 作用：将 862 个特征通道物理剥离，转化为独立的单维样本，切断空间耦合
            x_enc = x_enc.permute(0, 2, 1).reshape(B * C, L, 1)
        
        # 1. 动态决定是否应用 RevIN 规范化 (此时处理的是单维张量)
        ...
        # 维度对齐，准备进入 1D 卷积
        x = x.permute(0, 2, 1)  # (B*C, 1, L)
        
        # 2. 经过精妙的 FFT + DCN 骨干网络 (Stem层现在面对的是合法的 1 通道输入)
        x = self.forward_features(x)
        x = x.reshape(x.shape[0], -1)  # flatten，形状变为 (B*C, dims[-1] * last_len)
        ...
        if self.task_name in ['anomaly_detection', 'imputation']:
            ...
            # 输出形状重建为 (Batch * Channel, pred_len, 1)
            x = x.reshape(x.shape[0], self.pred_len, self.enc_in)

            # 预测任务需要逆平稳化还原量纲
            x = self.revin(x, mode='denorm')
            
            # --- 核心干预 2：张量展开，重构回原始的高维物理空间 ---
            # (Batch * Channel, pred_len, 1) -> (Batch, Channel, pred_len) -> (Batch, pred_len, Channel)
            x = x.reshape(B, self.c_out, self.pred_len).permute(0, 2, 1)
