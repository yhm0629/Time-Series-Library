...
def build_norm_layer(dim,
    ...
    if norm_layer == 'BN':
        ...
        # 修改点：从 BatchNorm2d 改为 BatchNorm1d
...
class Block(nn.Module):
    def __init__(self, dim,
        ...
        # 替换为卷积版本 MLP
        self.mlp = Conv1dMLP(in_channels=dim,
        ...
        # 使用文件末尾定义的自定义 LayerNorm，原生支持 channels_first
        self.norm1 = LayerNorm(dim, eps=1e-6, data_format="channels_first")
        self.norm2 = LayerNorm(dim, eps=1e-6, data_format="channels_first")

    def forward(self, x):
        # x 形状: (Batch, Channel, Length)
        
        # 第一阶段：RFA 模块
        # 内部已包含 norm1 的逻辑（基于原代码 ConvMod 实现）
        x = x + self.drop_path(self.layer_scale.unsqueeze(-1) * self.rfa(x))
        
        # 第二阶段：卷积 MLP 模块
        # 移除原有的 permute(0, 2, 1) 转换，直接在 (B, C, L) 维度操作
        # gamma2 需扩展维度以匹配广播机制: (Channel, 1)
        x = x + self.drop_path(self.gamma2.unsqueeze(-1) * self.mlp(self.norm2(x)))
